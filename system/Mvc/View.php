	...
	public function render($view = null)
	{
		...
		if (SIDE == 'backend')
		{
			if (Document::getTitle())
				Document::setTitle(Document::getTitle() . ' • ' . \Config::get('sitename', 'Vanda'));
			else
				Document::setTitle(\Config::get('sitename', 'Vanda'));
		...
		else
		{
			...
			if (!Document::getMetaKey())
				Document::setMetaKey(\Config::get('metakey'));

			if (!Document::getMetaDesc())
				Document::setMetaDesc(\Config::get('metadesc'));
		}
		...
		{
			$head = '<title>' . Document::getTitle() . '</title>';

			if (Document::getGenerator())
				$head .= "\n\t\t" . '<meta name="generator" content="' . Document::getGenerator() . '" />';

			if (Document::getMetaKey())
				$head .= "\n\t\t" . '<meta name="keywords" content="' . Document::getMetaKey() . '" />';

			if (Document::getMetaDesc())
				$head .= "\n\t\t" . '<meta name="description" content="' . Document::getMetaDesc() . '" />';
			...
			$data = str_replace('{{title}}', Document::getTitle(), $data);
