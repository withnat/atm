					...
					foreach ($arr1s as $arr1)
					{
						...
						elseif ($key == 'requiredif' and @$arr2s[1])
						{
							...
							if ($explode)
							{
								...
								$rules[$name]['required'] = 'function() { return $.trim($("#' . $arr[0] . '").val()) ' . $operator . ' "' . $arr[1] . '"; }';
							}
...
//						}
						elseif ($key == 'unique' and strpos(@$arr2s[1], '/'))
						{
							...
							else
							{
								...
								$messages[$name]['remote'] = 'function() { return "Is already in use."; }';
							}
						}
						...
						elseif (in_array($key, ['email', 'number']))
						{
							$rules[$name][$key] = 'true';
	...
	{
		if (trim($label) == '')
			$label = '<i class="fa fa-refresh"></i> ';// . t('Reset');
	...
	{
		if (trim($label) == '')
			$label = '<i class="fa fa-check-square-o"></i> ' . t('Save');
	...
	{
		if (trim($label) == '')
			$label = '<i class="fa fa-times"></i> ' . t('Close');
	...
	{
		if (trim($label) == '')
			$label = '<i class="fa fa-arrow-left"></i> ' . t('Back');
	...
	private static function _getOptionFromDB($id, $columnValue = null, $columnText = null, $where = null)
	{
		$output = [];
